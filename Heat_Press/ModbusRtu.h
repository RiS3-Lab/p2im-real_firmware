...
/*
    // if received new bytes, update timeout value by T35(5ms)
    // check T35 after frame end or still no frame end
    if (u8current != u8lastRec)
    {
        u8lastRec = u8current;
    ...
    // no new bytes received since last poll, wait until timeout
    if (millis() < u32time) return 0;

    // timeouts, i.e. in last T35(5ms), no new bytes received
    // this indicates modbus protocol is designed for low speed operation
    u8lastRec = 0;
*/
